...
// Generators
+ (void)socketWithHost:(NSString *)hostURL token:(NSString *)token response:(void (^)(SIOSocket *))response {
...
+ (void)socketWithHost:(NSString *)hostURL reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout token:(NSString *)token response:(void (^)(SIOSocket *))response {
    return [self socketWithHost: hostURL
         reconnectAutomatically: YES
                   attemptLimit: -1
                      withDelay: 1
